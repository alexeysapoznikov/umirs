...
?>
    ...
    <div class="catalog-block">
        ...
        $tests = mysqli_query($dbconn, "SELECT * FROM tests");
            ...
            ?>
            <div class="catalog-item" id="<?= $test['name']; ?>">
    ...
    <script>
        $(document).ready(function() {
            $('#search').on('input', function() {
                const searchText = $(this).val().toLowerCase(); // Получаем значение из поля ввода

                $('.catalog-block').children().each(function() {
                    const elementId = $(this).attr('id').toLowerCase(); // Получаем id текущего элемента

                    if (elementId.includes(searchText)) {
